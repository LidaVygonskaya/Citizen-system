        ...
        for relative_id in relatives:
            ...
            if relative_model is None:
                raise serializers.ValidationError(
                    f"Citizen with citizen_id {relative_id}" f" is not presented in import."
                )

            if current_citizen_id not in relative_model["relatives"]:
                raise serializers.ValidationError(
                    f"Citizen with {relative_id} does not have citizen with citizen_id"
                    f" {current_citizen_id} in his relatives"
                )
            if relative_id == current_citizen_id:
                raise serializers.ValidationError(
                    f"Citizen with id {current_citizen_id}" f" has self-related relatives field"
                )
        return attrs
